...
Kanana Skills는 카나나 관련 기능들을 Claude Code에서 RAG 기반으로 활용할 수 있도록 도와주는 플러그인 마켓플레이스입니다. 향후 다양한 카나나 스킬이 추가될 예정입니다.

## 포함된 스킬

| 스킬 | 커맨드 | 설명 |
|------|--------|------|
| **앰배서더 SNS 허브** | `/kanana_ambassador` | kanana.it.com 사이트 구조, API, 앰배서더 데이터 조회 (Read-Only) |
| **숙제 체크리스트** | `/kanana_homework` | 마감일 기반 숙제 관리, 추가/완료/조회 (로컬 저장) |
| *(예정)* | | 추가 카나나 스킬들 |

## How it works

카나나 앰배서더 관련 질문을 하면, `/kanana_ambassador` 스킬이 활성화됩니다. "카나나 사이트 구조 알려줘", "앰배서더 목록 보여줘", "API 명세 확인해줘" 같은 요청을 하면 스킬이 해당 레퍼런스 문서를 참조하여 정확한 답변을 제공합니다.
...
```
# Should see:
# /kanana_ambassador - 카나나 앰배서더 SNS 허브 도우미
```
